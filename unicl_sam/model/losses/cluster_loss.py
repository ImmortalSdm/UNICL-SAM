...
class DiscriminativeLoss(_Loss):
    ...
    def __init__(self, delta_var=0.5, delta_dist=3.5, # 1.5
    ...
    def _discriminative_loss(self, input):
        bs, n_clusters, n_features = input.size()
        #max_n_clusters = target.size(1)

        input = input.permute(0, 2, 1).contiguous() #.view(bs, n_features, height * width)
