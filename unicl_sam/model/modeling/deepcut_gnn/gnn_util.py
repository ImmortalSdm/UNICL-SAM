...
def my_graph_to_mask(S, image_tensor, image):
    # mask to original image size
    S = torch.reshape(S, (image_tensor.shape[2], image_tensor.shape[3]))
    mask = F.interpolate(S.unsqueeze(0).unsqueeze(0), (image.shape[2], image.shape[3]), mode='nearest')
    S = torch.reshape(S, (S.shape[0] * S.shape[0],)).type(torch.LongTensor)
    return mask, S
...
def create_adj(F, cut, alpha=1, ratio=0.5):
    W = (F @ F.T).float()
    # if NCut
    if cut == 0: 
        ...
        # threshold
        W = W * (W > 0)
        value, indice = W.topk(k, 1, True, True)
        W_new = torch.zeros_like(W, device=W.device)
        W_new = W_new.scatter(1, indice, value)
        # norm
        W = W_new / (W_new.max() + 1e-8)
    # if CC
    else:
        W = W - (W.max() / alpha + 1e-8)
...
def load_data(adj, node_feats):
    ...
    return node_feats, torch.stack(edge_index), edge_weight
