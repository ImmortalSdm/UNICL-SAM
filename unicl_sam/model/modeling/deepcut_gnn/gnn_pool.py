...
class GNNpool(nn.Module):
    ...
    def forward(self, data, A, return_feat=False):
        """
    ...
    def loss(self, A, S):
        ...
        mincut_loss = -(num / (den+1e-8))

        # orthogonality loss
        St_S = torch.matmul(S.t(), S)
        I_S = torch.eye(self.num_clusters, device=A.device)
        ortho_loss = torch.norm(St_S / (torch.norm(St_S)+1e-8) - I_S / (torch.norm(I_S)+1e-8))
        # print(ortho_loss)
